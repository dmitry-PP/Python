...
class Hero:
    ...
    def get_info(self,title):
        ...
        with open('totals.csv','a+',encoding='utf-8') as file:
            csv.writer(file).writerow(list(self.INFO.values()))
...
class Field(Widget):
    ...
    def _keyboard_closed(self):pass
    ...
    def _on_keyboard_down(self, keyboard, keycode, text, modifiers):#keycoad[1]
        ...
        match keycode[1]:
            ...
            case 'f1':
                self.__popup_saver("Сохранить",self.saver)

            case 'f5':
                self.__popup_saver("Загрузить",self.load_saver)
    ...
    def __popup_saver(self,msg,func):

        self.event_monstor.cancel()
        bx = BoxLayout(orientation='vertical')
        popup = Popup(title='Сохранение', content=bx, size_hint=[None, None], size=[350, 350])

        bx.add_widget(Label(text='Введите название сохранения:'))
        bx.add_widget(txt := TextInput())
        bx.add_widget(btn := Button(text=msg))
        btn.bind(on_press=partial(func, popup.dismiss, txt))

        popup.bind(on_dismiss=lambda *args: setattr(self, 'event_monstor',
                                                    Clock.schedule_interval(self._update_pos_monstors, 1)))
        popup.open()

    def saver(self,dismiss,ti,ins):
        # files = {os.path.splitext(file)[0] for file in next(os.walk('save'))[2]}

        with open(rf'save/{ti.text}.json','w',encoding='utf-8') as fp:
            obj : dict = self.my_hero.INFO.copy()

            obj['time'] = (datetime.now() - obj['time'] ).total_seconds()
            obj.update({
                        'hero_pos':(self.current_row,self.current_col),
                        'monsters_pos':list(self.monstors_dct.values()),
                        'coins_pos':[(int(self.Y - 1 - pos[1] // self.block_size[1]), int(pos[0] // self.block_size[0])) for pos in self.coins_dct.keys() ],
                        'exit':self.exit_coords})
    ...
    def load_saver(self,dismiss,txt,ins):
        files = {os.path.splitext(file)[0] for file in next(os.walk('save'))[2]}
        if txt.text not in files:
            return
        with open(rf'save/{txt.text}.json','r',encoding='utf-8') as fp:
            obj = json.load(fp)
            self.__current_row,self.__current_col = obj['hero_pos']
            self.field = obj['field']
            self.color_matrix=[]
            self.coins_dct = dict()
            self.monstors_dct = dict()
            self.my_hero=Hero(obj['hp'],(10,20))
            self.my_hero.INFO['coins'] = obj['coins']
            self.my_hero.INFO['monsters'] = obj['monsters']
            self.my_hero.INFO['time'] -= timedelta(seconds=obj['time'])
            self.__monstor_fight_popup()
            self.canvas.clear()
            self.canvas.after.clear()
            self.render()
            with self.canvas.after:
                self.hero = Rectangle(pos=(self.block_size[0]*self.__current_col,self.block_size[1]*(self.Y-1-self.__current_row)), size=self.block_size, source='hero.png')
            self.setup_coins(pos=obj['coins_pos'])
            self.setup_monstors(pos=obj['monsters_pos'])
            with self.canvas.after:
                exit = obj['exit']
                Rectangle(pos=(self.block_size[0]*exit[1],self.block_size[1]*(self.Y-1-exit[0])), size=self.block_size, source='exit.webp')
            self.exit_coords = tuple(exit)
    ...
    def __setup(self,source,count=5,pos=None):
        data=[]
        if pos is None:
            ...
            for xy in pos:
                with self.canvas.after:
                    rc = Rectangle(pos=(xy[1]*self.block_size[0],(-xy[0]+self.Y-1)*self.block_size[1]),size=self.block_size,source = source)
                data.append((rc.pos, rc))
        return data

    def setup_coins(self,count=5,pos=None):
        coins=self.__setup('coin.png', count=count,pos=pos)
        self.coins_dct.update(dict(coins))

    def setup_monstors(self,count=5,pos=None):
        monstors=self.__setup('monstor.png',count=count,pos=pos)
